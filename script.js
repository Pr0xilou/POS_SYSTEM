...
function add_order() {
    ...
    var forms = interactableDiv.querySelectorAll('.adding_removing select'); // Select all dropdown menus in added forms

    var availableOptions = ['Burger', 'Fries', 'Fishbol', 'Kikiam'];

    // Remove selected options from availableOptions
    ...
    form.className = 'adding_removing';
    ...
    // Add available options to the dropdown menu
    availableOptions.forEach(function(optionText) {
...
function remove_order() {
    ...
    var interactableDiv = document.querySelector('.interactable');
    var forms = interactableDiv.querySelectorAll('.adding_removing');
...
/* Paying Out */
// The final computation shows the ampount of what you paid and amount of your change 
function pay_out() {
    ...
    var paidAmount = parseFloat(cashInput);
    var change = paidAmount - totalAmount;

    if (change < 0) {
        alert("Insufficient cash. Please provide more cash.");
    } else {
        alert("Paid Amount: " + paidAmount.toFixed(2) + " pesos\nChange: " + change.toFixed(2) + " pesos");
    }
}
